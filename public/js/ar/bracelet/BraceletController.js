(function(global) {
    ...
    global.EnsureAR.Bracelet.BraceletController = class BraceletController {
        constructor(scene) {
            this.scene = scene;
            this.mapper = new global.EnsureAR.Bracelet.BraceletMapper();
        ...
        update(handResult, modelNode, occluderNode, modelMeta, W, H) {
            ...
            if (!mapData) {
                if (ts - this.lastValidTime < CONFIDENCE_TIMEOUT_MS && this.lastValidState) {
                    ...
                    if (global.DEBUG_AR) {
                        this.debugRenderer.draw(this.lastValidState.raw, this.lastValidState.model.position, 'bracelets');
                    } else {
            ...
            if (global.DEBUG_AR) {
                this.debugRenderer.draw(mapData, finalState.model.position, 'bracelets');
            } else {
                this.debugRenderer.hide();
            }
