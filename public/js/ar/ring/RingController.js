(function (global) {
    ...
    global.EnsureAR.Ring.RingController = class RingController {
        constructor(scene) {
            this.scene = scene;
            this.mapper = new global.EnsureAR.Ring.RingMapper();
        ...
        update(handResult, modelNode, occluderNode, modelMeta, W, H) {
            ...
            if (!mapData) {
                if (ts - this.lastValidTime < CONFIDENCE_TIMEOUT_MS && this.lastValidState) {
                    ...
                    if (global.DEBUG_AR) {
                        this.debugRenderer.draw(this.lastValidState.raw, this.lastValidState.model.position, 'rings');
                    } else {
                    ...
                    return { isTracking: false, missingMsg: 'Hand not visible' };
                }
            }
            ...
            if (global.DEBUG_AR) {
                this.debugRenderer.draw(mapData, finalState.model.position, 'rings');
            } else {
                this.debugRenderer.hide();
            }
